...
// Imprime long
void print_long(const char *name, long int v) {
    printf("%s = %ld\n", name, v);
}
...
int main(int argc, char *argv[]) {
    ...
    // Datos de entrada
    // =============================
    x = (long int)(rand() % LIMITE);   // 0..255
    y = (long int)(rand() % LIMITE);   // 0..255
    if (y == 0) y = 1;                 // evitar division/mod 0
    ...
    // D) Bits
    // =================
    ux = (unsigned long int)x;
    uy = (unsigned long int)y;
